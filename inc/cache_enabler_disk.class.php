...
final class Cache_Enabler_Disk {
    ...
    private static function create_cache_files( $page_contents ) {
        ...
        foreach ( $cache_files as $new_cache_file ) {
            ...
            if (
                strpos( $new_cache_file_name, 'br' ) !== false
                && function_exists( 'brotli_compress' )
                && strpos( $new_cache_file_name, 'https' ) !== false
            ) {
                try {
    ...
    private static function write_cache_file( $cache_file, $page_contents ) {
        ...
        if ( ! self::mkdir_p( $cache_file_dir ) ) {
            return;
        }

        $cache_file_name = basename( $cache_file );

        // try to create new cache file
        $new_cache_file_created = file_put_contents( $cache_file, $page_contents, LOCK_EX );

        // set file permissions if successfully created
        if ( $new_cache_file_created !== false ) {
            clearstatcache();
            $new_cache_file_stats = @stat( $cache_file_dir );
            ...
            $page_created_url = self::get_cache_url( $cache_file_dir );
            $page_created_id  = url_to_postid( $page_created_url );
            $cache_created_index[ $cache_file_dir ]['url'] = $page_created_url;
            $cache_created_index[ $cache_file_dir ]['id']  = $page_created_id;
            $cache_created_index[ $cache_file_dir ]['versions'][ $cache_file_name ] = $new_cache_file_created;
